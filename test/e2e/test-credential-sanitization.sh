...
# Run the sanitization logic using the REAL production credential-utils module.
# This exercises the same stripCredentials + CREDENTIAL_SENSITIVE_BASENAMES that
# createSnapshotBundle() uses in nemoclaw/src/commands/migration-state.ts.
info "C1-C5: Running credential sanitization on mock bundle..."
sanitize_result=$(cd "$REPO" && node -e "
const fs = require('fs');
const path = require('path');
const { stripCredentials, CREDENTIAL_SENSITIVE_BASENAMES } = require('./nemoclaw/dist/credential-utils.js');

function walkAndRemoveCredentialFiles(dirPath) {
  let entries;
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (stat.isDirectory()) {
        walkAndRemoveCredentialFiles(fullPath);
      } else if (CREDENTIAL_SENSITIVE_BASENAMES.has(entry)) {
...
// 1. Remove credential-sensitive files (matches copyDirectory filter in production)
const agentsDir = path.join(bundleDir, 'agents');
if (fs.existsSync(agentsDir)) {
  walkAndRemoveCredentialFiles(agentsDir);
}

// 2. Sanitize openclaw.json — mirrors production sanitizeConfigFile():
//    delete gateway entirely, then strip credential fields from the rest
...
# NOTE: production sanitizeConfigFile() deletes the entire 'gateway' section
# (it contains auth tokens regenerated by the sandbox entrypoint), so we check
# that nvidia.apiKey is stripped AND that gateway is absent.
...
nvidia_apikey=$(echo "$config_content" | node -e "
  const d = require('fs').readFileSync(0, 'utf8');
  try { const c = JSON.parse(d); console.log((c.nvidia || {}).apiKey || 'MISSING'); }
  catch { process.exitCode = 1; }
" 2>/dev/null || echo "PARSE_ERROR")

gateway_present=$(echo "$config_content" | node -e "
  const d = require('fs').readFileSync(0, 'utf8');
  try { const c = JSON.parse(d); console.log('gateway' in c ? 'PRESENT' : 'ABSENT'); }
  catch { process.exitCode = 1; }
" 2>/dev/null || echo "PARSE_ERROR")
...
if [ "$gateway_present" = "ABSENT" ]; then
  pass "C3b: gateway section deleted (matches production sanitizeConfigFile)"
else
  fail "C3b: gateway section still present — production deletes it entirely"
fi

# C4: Non-credential fields must be preserved
info "C4: Checking non-credential field preservation..."
model_primary=$(echo "$config_content" | node -e "
  const d = require('fs').readFileSync(0, 'utf8');
  try { const c = JSON.parse(d); console.log(((c.agents || {}).defaults || {}).model?.primary || 'MISSING'); }
  catch { process.exitCode = 1; }
" 2>/dev/null || echo "PARSE_ERROR")

if [ "$model_primary" = "nvidia/nemotron-3-super-120b-a12b" ]; then
  pass "C4: agents.defaults.model.primary preserved"
else
  fail "C4: agents.defaults.model.primary corrupted (got: $model_primary)"
fi
...
# Run the real CREDENTIAL_SENSITIVE_BASENAMES-based removal — it should skip symlinks
# This mirrors how production copyDirectory() filters files via the basenames set.
c8_result=$(cd "$REPO" && node -e "
const fs = require('fs');
const path = require('path');
const { CREDENTIAL_SENSITIVE_BASENAMES } = require('./nemoclaw/dist/credential-utils.js');
...
function walkAndRemoveCredentialFiles(dirPath) {
  ...
  for (const entry of entries) {
    ...
    try {
      const stat = fs.lstatSync(fullPath);
      if (stat.isSymbolicLink()) continue;  // SKIP SYMLINKS — security critical
      if (stat.isDirectory()) {
        walkAndRemoveCredentialFiles(fullPath);
      } else if (CREDENTIAL_SENSITIVE_BASENAMES.has(entry)) {
        fs.rmSync(fullPath, { force: true });
...
walkAndRemoveCredentialFiles('$BUNDLE_SYM_DIR');
...
# C9-C11 use the real computeDigest() from credential-utils.ts to verify
# that digest comparison works correctly (empty, wrong, correct).
# C9: Empty digest string must be treated as a FAILURE
info "C9: Testing empty digest rejection..."

c9_result=$(cd "$REPO" && node -e "
const { computeDigest } = require('./nemoclaw/dist/credential-utils.js');

// Verify that empty/undefined digests are detectable —
// the caller (createSnapshotBundle) must reject them.
const emptyDigest = '';
const undefinedDigest = undefined;

// Empty string is falsy — the original bug let this slip through
if (!emptyDigest || emptyDigest.trim() === '') {
  console.log('REJECTED_EMPTY');
} else {
  console.log('ACCEPTED_EMPTY');
}

if (!undefinedDigest || (typeof undefinedDigest === 'string' && undefinedDigest.trim() === '')) {
...
const computed = computeDigest(content);
const wrongDigest = 'deadbeef0000000000000000000000000000000000000000000000000000dead';
console.log(computed !== wrongDigest ? 'REJECTED_WRONG' : 'ACCEPTED_WRONG');
" 2>&1)
...
console.log(digest === recomputed ? 'ACCEPTED_CORRECT' : 'REJECTED_CORRECT');
" 2>&1)
...
# Uses the REAL stripCredentials + isCredentialField from credential-utils.ts
info "C12: Testing pattern-based credential field stripping..."

c12_result=$(cd "$REPO" && node -e "
const { stripCredentials } = require('./nemoclaw/dist/credential-utils.js');
...
# Uses the REAL stripCredentials from credential-utils.ts
