...
| Windows WSL2 | Docker Desktop (WSL backend), Qualified rootless Podman | Tested with limitations | P1 | No | Docker Desktop remains required for the explicit Experimental managed llama.cpp Qwen 3.6 recipe and Windows-host Ollama. WSL-local Ollama can use Docker Desktop or the qualified rootless Podman provider. On a Windows WSL N1x, the selected provider runs the same bounded CUDA workload and capacity query before the installed-model ranking can select qwen3.6:35b. Other Windows WSL Express installs use WSL-local Ollama. Dedicated N1x WSL CI and physical end-to-end validation are pending. |
...
| Other Windows-on-ARM GPU paths | Unsupported | Windows-on-ARM CPU paths run under WSL2 'tested with limitations'. The qualifying Windows WSL N1x paths remain pending dedicated CI and physical end-to-end validation. Other Windows-on-ARM GPU routes remain Unsupported. An unqualified container provider, multiple reported GPU rows, an implausible GPU name, or a failed provider-owned bounded CUDA proof remains rejected. See closed issue #4565. |
