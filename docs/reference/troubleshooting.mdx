...
A direct MCP change becomes pending and is marked as applied only after gateway health passes.
...
If the error reports an MCP reconciliation refusal, restore the managed MCP projection and retry recovery:
...
The supervisor quarantines relaunch after five unexpected gateway exits or failed replacement candidates within 60 seconds.
Invalid Hermes config can make each replacement exit and consume that crash budget.

The quarantine is deterministic, so retrying restart or recovery cannot clear it.
Inspect the Hermes log and correct the process or config failure.
Then restart the sandbox to reset the supervisor's crash budget:
...
If the sandbox still cannot start, rebuild it from registered configuration:
